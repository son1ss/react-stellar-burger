...
export default function BurgerIngredients() {
  ...
  const handleScroll = () => {
    const scrollPosition = scrollContainer.current ? scrollContainer.current.scrollTop : 0
  ...
  return (
    <section className="pt-10 main-block">
      ...
      <div className={styles.categories}>
        {categories.map(item => {
          const id = (Object.keys(item) as (keyof typeof item)[])[0]
          const name = item[id]
      ...
      <div onScroll={handleScroll} ref={scrollContainer} className={`custom-scroll pt-10 ${styles.ingredientsPanel}`}>
        {categories.map(category => {
          const categoryName = (Object.keys(category) as (keyof typeof category)[])[0] 
          return (
            <Fragment key={categoryName}>
              ...
              <div className={`pt-6 pb-10 pl-4 pr-1 ${styles.ingredients}`}>
                {ingredients && ingredients.filter(ingredient => ingredient.type === categoryName).map(ingredient => (
                  <Ingredient 
                    key={ingredient._id}
