...
export const getPrice = (orderIngredients: Record<string, number>, ingredients: Ingredient[]) => {
  let price = 0;
...
export function Preview(props: React.HTMLProps<HTMLImageElement> & { overlay?: boolean; excess?: number }) {
  return (
    <div className={[styles.container, props.className].join(' ')}>
      <img src={props.src} alt={props.alt} className={styles.preview} />
      {props.overlay && (
        <p className={`text text_type_main-small text_color_primary ${styles.previewOverlay}`}>+{props.excess}</p>
      )}
    </div>
  );
}

function IngredientsRow({ ids, ingredients }: { ids: string[]; ingredients: Ingredient[] }) {
  ...
  return (
    <div className={styles.previews}>
      {usedIngredients[5] && (
        <Preview
          key={usedIngredients[5]._id}
          src={usedIngredients[5].image}
          alt={usedIngredients[5].name}
          overlay={excess > 1}
          excess={excess}
...
export default function Order({ createdAt, ingredients: orderIngredients, name, number, _id }: Order) {
  ...
  return (
    ...
    >
      <div className={styles.row}>
        ...
        <p className="text text_color_primary text_type_main-default">
