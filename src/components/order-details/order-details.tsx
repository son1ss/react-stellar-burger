...
export default function OrderDetails() {
  ...
  const ingredientsList = [bun._id, ...fillings.map((filling) => filling._id), bun._id];
  const [order, { data: burger, isError }] = useCreateOrderMutation();
  ...
  return (
    <div className={styles.details}>
      <p className={`text text_type_digits-large pt-4 pb-8 ${styles.id}`}>
        {!burger ? '...' : burger.order.number}
      </p>
      ...
      <p className="pb-30 text text_type_main-default text_color_inactive">
